...
public enum GSPagerViewTransformerType {
    case crossFading
    case zoomOut
    case depth
    case overlap
...
open class GSPagerViewTransformer: NSObject {
    
    open internal(set) weak var pagerView: GSPagerView?
    open internal(set) var type: GSPagerViewTransformerType
    
    open var minimumScale: CGFloat = 0.65
    open var minimumAlpha: CGFloat = 0.6
    
    public init(type: GSPagerViewTransformerType) {
        self.type = type
        switch type {
        case .zoomOut:minimumScale = 0.85
        case .depth: minimumScale = 0.5
        default:
            break
        }
    }
    ...
    open func applyTransform(to attributes: GSPagerViewLayoutAttributes) {
        guard let pagerView = pagerView else { return }
        let position = attributes.position
        let direction = pagerView.direction
        ...
        case .crossFading:
            ...
            switch direction {
            case .horizontal: transform.tx = -itemSpacing * position
            case .vertical: transform.ty = -itemSpacing * position
            }
        ...
        case .zoomOut:
            ...
            case -1 ... 1 :  // [-1,1]
                ...
                alpha = self.minimumAlpha + (scaleFactor - minimumScale) / (1 - minimumScale) * (1 - minimumAlpha)
            default:
                break
            }
            attributes.do {
        ...
        case .ferrisWheel, .invertedFerrisWheel:
            ...
            attributes.do {
                $0.alpha = abs(position) < 0.5 ? 1 : self.minimumAlpha
                $0.transform = transform
                $0.zIndex = zIndex
            }
        case .cubic:
            ...
            case -1 ..< 1:
                ...
                case .horizontal:
                    ...
                    attributes.center.x += directionValue*radius * 0.5 // ForwardX
                    transform3D = CATransform3DRotate(transform3D, theta, 0, 1, 0) // RotateY
                    transform3D = CATransform3DTranslate(transform3D,-directionValue*radius * 0.5, 0, 0) // BackwardX
                case .vertical:
                    // ForwardY -> RotateX -> BackwardY
                    attributes.center.y += directionValue*radius * 0.5 // ForwardY
                    transform3D = CATransform3DRotate(transform3D, theta, 1, 0, 0) // RotateX
                    transform3D = CATransform3DTranslate(transform3D,0, -directionValue*radius * 0.5, 0) // BackwardY
                }
                attributes.transform3D = transform3D
    ...
    open func proposedInteritemSpacing() -> CGFloat {
        ...
        let direction = pagerView.direction
        switch self.type {
        case .overlap: return direction == .horizontal ? pagerView.itemSize.width * -self.minimumScale * 0.6 : 0
        case .linear: return direction == .horizontal ? pagerView.itemSize.width * -self.minimumScale * 0.2 : 0
