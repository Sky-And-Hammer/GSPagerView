...
class GSPagerViewLayout: UICollectionViewLayout {
    ...
    internal var direction: GSPagerViewScrollDirection = .horizontal
    
    open override class var layoutAttributesClass: AnyClass {
        get {
            return GSPagerViewLayoutAttributes.self
        }
    }
    
    fileprivate var pagerView: GSPagerView? {
        return self.collectionView?.superview?.superview as? GSPagerView
    }
    ...
    override open func prepare() {
        ...
        self.direction = pagerView.direction
        self.leadingSpacing = self.direction == .horizontal ? (collectionView.frame.width-self.actualItemSize.width)*0.5 : (collectionView.frame.height-self.actualItemSize.height)*0.5
        self.itemSpacing = (self.direction == .horizontal ? self.actualItemSize.width : self.actualItemSize.height) + self.actualInteritemSpacing
    ...
    override open func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        let numberOfItemsBefore = self.direction == .horizontal ? max(Int((rect.minX-self.leadingSpacing)/self.itemSpacing),0) : max(Int((rect.minY-self.leadingSpacing)/self.itemSpacing),0)
        ...
        let maxPosition = self.direction == .horizontal ? min(rect.maxX,self.contentSize.width-self.actualItemSize.width-self.leadingSpacing) : min(rect.maxY,self.contentSize.height-self.actualItemSize.height-self.leadingSpacing)
        ...
        while origin-maxPosition <= max(CGFloat(100.0) * .ulpOfOne * fabs(origin+maxPosition), .leastNonzeroMagnitude) {
            ...
            let attributes = self.layoutAttributesForItem(at: indexPath) as! GSPagerViewLayoutAttributes
    ...
    override open func layoutAttributesForItem(at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        let attributes = GSPagerViewLayoutAttributes(forCellWith: indexPath)
